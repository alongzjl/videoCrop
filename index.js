...
export default function(file,fn,mallId){
    ...
    .then(function(videoInfoList) {
        ...
        Ajax.postVIDEO('http://52.internal.rongyi.com/easy-smart-web/fileUpload/uploadFile',videoFile,mallId).then(res=>{
            let { url } = res.result.data
            data.url = url
            Ajax.postJSON('http://52.internal.rongyi.com/easy-smart-web/source/saveMerchantSource',data).then(res=>{
                fn&&fn()
            })
        })
    });
...
function videCropImg(result){
    return new Promise(resolve => {
        Ajax.postVIDEO('http://52.internal.rongyi.com/easy-smart-web/fileUpload/uploadFile',result).then(res=>{
            if(res.success){
